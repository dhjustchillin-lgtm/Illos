...
static void CB2_InitBattleInternal(void)
{
    ...
    SetUpBattleVarsAndOliveZigzagoon();
...
u8 IsRunningFromBattleImpossible(enum BattlerId battler)
{
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_FIRST_BATTLE) // Cannot ever run from saving Olive's battle.
    {
        gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_DONT_LEAVE_OLIVE;
        return BATTLE_RUN_FORBIDDEN;
    }
